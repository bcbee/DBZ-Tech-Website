...
docker image push gcr.io/api-project-367056975125/dbz-tech-website:latest'''
    ...
    stage('Deploy') {
      steps {
        ...
        sh 'docker container run -e "GCP_JSON=$(cat $GCP_SERVICE_ACCOUNT)" google/cloud-sdk bash -c "env && echo "$GCP_JSON" && cat /gcp_service_account.json && gcloud auth activate-service-account --key-file /gcp_service_account.json && gcloud container clusters get-credentials dbz-arterion && kubectl get pods"'
      }
    }
