...
docker image push gcr.io/api-project-367056975125/dbz-tech-website:latest'''
    ...
    stage('Deploy') {
      steps {
        ...
        sh 'docker container run -e "GCP_SERVICE_ACCOUNT=$(cat $GCP_SERVICE_ACCOUNT)" google/cloud-sdk gcloud version; echo $GCP_SERVICE_ACCOUNT > /gcp_service_account.json; gcloud auth activate-service-account --key-file /gcp_service_account.json; gcloud container clusters get-credentials dbz-arterion; kubectl get pods  '
      }
    }
