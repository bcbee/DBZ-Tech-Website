...
class search {
    public static function process($input) {
        $xml = file_get_contents('http://api.wolframalpha.com/v2/query?appid=L89E6K-PT8PGUR4X4&input='.urlencode($input).'&format=html,image,plaintext,sound');
        #echo $xml;
        ...
        if (!$output == "") {
            foreach($output as $section) {
                print_r($section['markup']);
            }
            $refined = array_search('markup', $output);
            print_r($refined);
        } else {
            echo "No Results!";
        }
